...
public static class VideoProcessorService
{
    public static async Task TrimVideo(string path)
    {
        string tempVideoPath = Path.GetTempPath();

        string tempFramePath = Path.Combine(Path.GetTempPath(), "frames");
        (tempVideoPath, string blobName) = await StorageService.DownloadOriginalVideo(tempVideoPath);
        ...
        await ExtractFrames(video, tempFramePath);
    ...
    public static async Task ExtractFrames(Video video, string tempFramePath)
    {
        IMediaInfo videoInfo = await FFmpeg.GetMediaInfo(video.FilePath);
    ...
    public static NamedOnnxValue ConvertToOnnx(string framePath, string inputName = "images", int targetSize = 640)
    {
    ...
    public static Video DetectPlates(Video video, string framePath)
    {
    ...
    public static int GetTrimStart(Video video)
    {
        float confidence = 0f;
        float yValue = new();
        int i = 0;
    ...
    public static int GetTrimFinish(int trim, Video video)
    {
        float confidence = 0f;
        float yValue = new();
