...
using BarClip.Maui.Platforms.Windows.Helpers;
using BarClip.Models.Requests;
using FFMpegCore;
using static BarClip.Core.Helpers.FileHelper;

namespace BarClip.Maui.Platforms.Windows.Services;

public class WindowsVideoEditor : IVideoEditor
{
    private readonly FrameService _frameService;
    private readonly PlateAnalysisService _plateAnalysisService;

    public WindowsVideoEditor(FrameService frameService, PlateAnalysisService plateAnalysisService)
    {
    ...
    public async Task<ProcessedVideoRequest> ProcessVideo(SessionFolderPaths sessionFolderPaths, OriginalVideoRequest video)
    {
        ...
        try
        {
            ...
            await FFMpegHelper.TrimAndLabelAsync(video, processedVideo, weightText);
    ...
    public async Task<List<BarClip.Models.Domain.Frame>> ExtractAndProcessFrames(OriginalVideoRequest originalVideo) // medium
    {
        ...
        try
        {
            await FFMpegHelper.ExtractAllFramesAsync(originalVideo, tempFramePath);
        }
        catch (Exception ex)
        {
    ...
    {
        return await FFMpegHelper.MergeVideos(sessionFolderPaths, sessionId);
    }
    public async Task<string[]> ExtractThumbnails(string originalFolderPath, string thumbnailFolderPath)
    {
        return await FFMpegHelper.ExtractThumbnails(originalFolderPath, thumbnailFolderPath);
    }
}
